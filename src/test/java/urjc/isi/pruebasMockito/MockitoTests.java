...
public class MockitoTests {
	...
	public void MockWithUnspecifiedArguments(){
		...
		verify(c).compareTo(5); //el metodo verify, verifica que si tu objeto c (mockito) ha sido llamado mediante el metodo, en este cas  (compareTo)
		verify(c).compareTo(3);
		verify(c, never()).compareTo(25);
		verify(c, times(1)).compareTo(5);
		verify(c, atLeastOnce()).compareTo(5);
		verify(c, atLeast(2)).compareTo(5);
	}
